...
The measurements can be made between cameras poses, GCPs or undeclared points.
Topo adjustment tries to initialize the undeclared points based on known points and observations.
If automatic coordinates initialization succeeds the new points are added to the first GCP set of the adjustment
(if an output dir is given).

The command \texttt{OriBundleAdj} can use topo measurements in compensation via the optional parameter \texttt{TopoDirIn}. Optional parameter \texttt{TopoDirOut} can be used to get a detailed list of observations with their residuals.

\texttt{TopoDirIn} can be used with photogrammetric constraints, or even without images nor ORIs.
In this case, the command line should be like:

\begin{lstlisting}
MMVII OriBundleAdj NONE NONE NONE GCP3D=[[InitRTL,1.,FinalRTL]] TopoDirIn=Obs1 TopoDirOut=Obs1_out
\end{lstlisting}
...
To separate newly initialized points from previous GCPs, an empty GCP set can be given as first \texttt{GCP3D}:
\begin{lstlisting}
MMVII OriBundleAdj NONE NONE NONE GCP3D=[[NONE,1.,NewPts], [InitRTL,1.,FinalRTL]] TopoDirIn=Obs1
\end{lstlisting}
...
\begin{itemize}
    ...
    \item measurement \textit{a priori} absolute precision $\sigma_{abs}$ (in meters for distances, in gon for angles)
    \item optinal \textit{a priori} relative precision $\sigma_{rel}$
    \item anything else is ignored until the end of the line
\end{itemize}

Example of an \texttt{OBS} line describing a measured distance of 100.0000 m, with a $\sigma$ of 1 mm + 1 PPM from \texttt{PointA} to \texttt{PointB}:

\begin{lstlisting}
    3   PointA    PointB    100.0000    0.001   0.000001 * comment
\end{lstlisting}
...
The total precision of an observation depends on its type and lenght:
$$\sigma_{total} = \sigma_{abs} + \sigma_{rel} \cdot len$$ for distances,
$$\sigma_{total} = \sigma_{abs} + \sigma_{rel} / len$$ for angles.
...
Since there are no images, we give the special pattern "NONE", and any name for ORI in and out are accepted, as there are no cameras.
...
We will describe the input files equivalent for the \texttt{OriBundleAdj} command.
