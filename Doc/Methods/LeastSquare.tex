...
\subsection{cLinearWeighterResidual}

Is a simple Weighter function designed to have standard least-squares weighting for correct measurements and a linear attenuation for suspicious measurements.

Is is aimed at keeping a big number of observations on early stages of adjustment, when parameters are not be well initialized.
...
\begin{itemize}
  \item $\sigma$: the classical variance
  \item $Thrs1$: the threshold below which the observation weight is $1/\sigma^2$
  \item $Thrs2$: the threshold above which the observation weight is 0
\end{itemize}

Between $Thrs1$ and $Thrs2$, the observation weight decreases linearly.

\begin{figure}[H]
\centering
\includegraphics[width=10cm]{Methods/Images/cLinearResidual}\caption{cLinearWeighterResidual $w(r)$ representation}
        \label{fig:cLinearWeighterResidual}
\end{figure}
...
set samples 1000
set yrange [-0.1 : 1.1]
w(r, sigma, th1, th2) = (abs(r) > th2) ? 0 : (abs(r) < th1) ? (1 / sigma**2) : (1 / sigma**2)*(1-(abs(r)-th1)/(th2-th1))
set xlabel "r"
set ylabel "w(r)"
plot [-th2-1:th2+1] \
    w(x, 1, 1, 3) with lines title "sigma=1, Thrs1=1, Thrs2=3"
\end{comment}
