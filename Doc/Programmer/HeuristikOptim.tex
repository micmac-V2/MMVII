...
In section~\ref{HeurOpt:OBS} we present methods and classes for computing
local extrema of "any" function, in~\ref{Sec:Samp:Sphere} we present methods and classes
for sampling the spheres, in~\ref{HeurOpt:Man:Intro} we present classes and methods
for global optimization of functions on bounded manifolds.


%---------------------------------------------
%---------------------------------------------
%---------------------------------------------
\section{Discret optimization with class {\tt cOptimByStep}}
...
\begin{itemize}
    ...
    \item  at second step, $F$ is evaluated on  the $3$  points  arround $P_1$ were not already in $V_0$ (i.e $V_1$);
    \item  at second step, $F$ is evaluated on  the $5$  points  arround $P_2$ were not already in $V_1$ (i.e. $V_2$).
\end{itemize}
...
enough to the global minimal.  As the previous $\MM$  are
...
\begin{itemize}
   ...
   \item  let $f$ be the number of points in each dimension and each face,  we make a $f-adic$
          decomposition of  $F^k$ to have the coordinate inside the face.
\end{itemize}
...
In this context,  the calibration of a clino-meter is a unit vector, so a point of $\SPH^2$, and the function to minimize
is the sum of residual that would be computed from a given calibration.  There is two main variant on
this basic case :
...
\begin{itemize}
   \item start we the global maxima $E_1$;
    \item each time a new maxima $E_k$ is found, mark as reached all the point of the sampling
          such as $D_{\MM}(P,E_k) < \delta$  where $\delta$ is a threshold and $D_{\MM}$ is
...
We describe here the requirements that a class must comply with to a descriptor of
manifold~\footnote{typicall the concept of \CPP20 may be adapted to describe this}.
Note that most current manifold being already described, for the user it's not mandatory 
...
\begin{itemize}
    ...
    \item {\tt  typedef cP3dNormWithUK  tDescrTgtSp;}  the type used as descriptor of tangent space, i.e. 
...
\begin{itemize}
    ...
    \item then {\tt CartProduct\_OptimDisc<T1,T2>} will be class for describing
           $\MM_1 \times \MM_2$;
