...
class cPointCloud
{
  public:
	...
	void AddPtColored(const cPt3dr& aPt0, const tU_INT2& color, const tU_INT1& leaf);
	/*
			{
				 if (mMode8)
	...
	*/
	...
	tU_INT2 KthDegVis(int aK) const { return mDegVis.at(aK); }
	tU_INT1 KthSzLeaf(int aK) const { return mSzLeaves.at(aK); }
	std::vector<tU_INT1> KthColor(int aK) const { return mColors.at(aK); }
	...
	void UpdateFineDensity(); ///<  longer but adapted to cloud with non rectangular footprint
