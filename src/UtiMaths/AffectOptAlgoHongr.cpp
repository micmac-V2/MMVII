...
tINT4 CostPermutation(cIm2D<tINT4> aCostM,const std::vector<int> & aPerm)
{
   tINT4 aRes = 0;
...
/// Test is the vector of int represent of permutauion
bool IsPermutation(const std::vector<int> & aPerm)
{
...
/// Check that aVAll is the vector of all permutation of aNb
...
/// Fill aRes with all permutation of size aNb,  efficient in time but not in memmory
void ComputeAllPermutation(std::vector<std::vector<int>> & aRes, int aNb)
{
    aRes.reserve(iFactorial(aNb));
    aRes = {{0}}; // Inititialise with permutaion of set{0}

    // parse the size of permutation
   for (int aNbSub=1; aNbSub<aNb ; aNbSub++)
   {
       std::vector<std::vector<int>>  aResPrec = aRes;
       aRes.clear();
       // Pars all permutaion of [0,aNbSub-1]
       for (const auto & aPerm : aResPrec)
       {
           // Insert element at all possible position
           for (int aPosNew=0 ; aPosNew<= aNbSub ; aPosNew++)
           {
               aRes.push_back({});
               for (int aK=0; aK<=aNbSub ; aK++)
               {
                   // if it's right position add new element
                   if (aK==aPosNew)
                       aRes.back().push_back(aNbSub);
                   // add previous elemnt if not at end
                   if (aK!=aNbSub)
                       aRes.back().push_back(aPerm.at(aK));
               }
...
void OneBench_AlgoHongrois(int aNb)
{
    //  generate a random matric
   cPt2di aSz(aNb,aNb);
   cIm2D<tINT4> aCostM(aSz);
   auto & aDIm =  aCostM.DIm();

   for (const auto & aPix : aDIm)
       aDIm.SetV(aPix,RandUnif_N(10000));


   // extract sol with ungarian method
  cIm1D<tINT4>  aSolMat = OptAffect_HongrAlgo(aCostM);
  std::vector<int> aSolVect = cDenseVect<tINT4>(aSolMat).ToStdVect();
  tINT4 aCostH = CostPermutation(aCostM,aSolVect);

  // extract cost of best sol with combinatorial exploration of all perm
  std::vector<std::vector<int>> aVecAllP;
  ComputeAllPermutation(aVecAllP,aNb);
  int aMinCost= 1e9;
  for (const auto & aPerm : aVecAllP)
      UpdateMin(aMinCost,CostPermutation(aCostM,aPerm));
...
void Bench_AlgoHongrois(cParamExeBench & aParam)
{
    ...
    // Chexk that permut generator is ok
    for (size_t aK=1 ; aK<=6 ; aK++)
    {
       std::vector<std::vector<int>> aPerms;
       ComputeAllPermutation(aPerms,aK);
       BenchAllPerm(aPerms,aK);
      // StdOut() << aPerms << "\n";
    }


    for (size_t aSz=1 ; aSz<=8 ; aSz++)
    {    for (int aKTest=0; aKTest<10 ; aKTest++)
         {
            OneBench_AlgoHongrois(aSz);
         }
       // StdOut() << "SZ= " << aSz << "\n";
     }
...
/**
    This file contains an implementation of opimal affectation problem using the 
  "Hongrois" algorithm.  The algorithm was inpired from Gondran-Minoux, with many
  tricky detail added to have it efficient.
    The computation time is O(N^3) for a NxN matrix.


    It was my first programm in C, in 1986, with a programming style close to fortran  
  (all function are void, result in global variable, the index begin to 1) ...
  As it works, I have made a minimal effort to have it work on C++ compiler, w/o any
  strong refactoring.
...
template <class Type> Type * NewTab(int aSz)
{
   // MMVII_INTERNAL_ERROR("NewTab to re-write");
   return new Type [aSz];
}


#define NEW_TAB(SZ,TYPE) NewTab <TYPE>(SZ)

template <class Type> void DELETE_TAB(Type * aPtr) {delete [] aPtr;}
...
void ECRIBOOL() ;
void ECRICHEM();
void OptAffect_HongrAlgo(cIm2D<tINT4>,cIm1D<tINT4>)  ;
void LECTURE(cIm2D<tINT4>) ;
void ALGO1() ;
void ALGO2()   ;
void ALGO3() ;
void VERIF();
...
inline double quelle_heure_est_il()
{
   return 0.0;
}
...
MBU static double temps;             /* temps de calcul pour algo1,algo2,algo3 */
                                       
MBU static int Taille;                                 /*taille effective des matrices*/

MBU static int NbSom;                                  /*nombre effectif de sommets*/

MBU static int ** Matrice;              /*matrice des couts*/

MBU static int ** EcGra;      /*        graphe "d'ecart"; pour I appartient a   */
                                   /*  [ 1 , EcGra[0][J] ] :EcGra[I][J] contient les */  
                                   /*  machine telles que Matrice[I][J]=0            */   


MBU static int * Chemin;                       /*contient le chemin de S a P*/


MBU static int Existe;                                 /*=1 si existe chemin de S a P */ 


MBU static int * AffTache;                 /* contient eventuellement la machine */
                                       /*    a laquelle est affecte la tache; */
                                       /* =0 si aucune affectation           */                        

MBU static int * AffMach;                  /* contient eventuellement la tache   */
                                       /* a laquelle est affectee la machine; */
                                       /* =0 si aucune affectation           */

MBU static int  *GrInjMax[2];           /* graphe injectif maximal issu de */     
                                    /*            l'affectation simple */ 

MBU static int * RecLigne;           /* contient les colonnes qui participent */
                                     /*     a un recouvrement               */


MBU static int * RecColon;               /* conttient les lignes qui participent */
                                     /*  a un recouvrement                   */

MBU static int AdrG2;                        /* adresse dans GrInjMax de du premier */
                                  /*           element de G2             */

MBU static int AdDeltGL;                     /* adresse dans RecLigne de la premiere */
                                  /*        ligne recouvrant Delta-GRAPHE */

MBU static int AdDeltGC;                     /* adresse dans RecColon de la premiere */
                                  /*    colonnne recouvrant Delta-GRAPHE  */

MBU static int * MemAdGIM;                 /* memorise les adresses dans GrInjMax */
                                  /* des elements qui appartiennent a une */
                                  /* rangee de recouvrement */

MBU static int FinRecObl;                    /* =1 si fin du recouvrement obligatoire */

MBU static int NbProLig;                     /* Nb provisoire de ligne recouvrantes */
                                  /* (compte tenu des lignes facultatives) */

MBU static int * MinDeCol;            /* contient pour Chaque colonne non */
                                  /* couvrante le minimum de ces elements */
                                  /* non couverts */


MBU static int MinGlob;                      /* = minimum de MinDeCol */

MBU static int * AddCol;              /* Quantite a additionner a chaque colonne */
 
MBU static int * AddLig;              /* Quantite a additionner a chaque ligne   */

MBU static int AddEns;                       /* Quantite a additionner a chaque element */

MBU static int NouvZero;                     /* =1 si le nombre de zeros independants a augmente */
                                  /* =0 sinon                                         */

MBU static int NbNvCoOb;                     /* = nombre de nouvelles colonnes obligatoires */

MBU static int NbIter;                       /* nombre d'iterations */

MBU static int ** MemMatr;    /* memorise la matrice de depart */

MBU static int MaxAbso;                      /* = Max des elements rencontres au cour de */
                                  /* l'algorithme                             */

MBU static int CompHon1;                     /* compte le nombre d'appel a Algo1 */

MBU static int CompArEx;                     /* compte le nombre d'arcs explores */

MBU static int ComElEx2;                     /* compte le nombre d'elements explores dans Algo2 */

MBU static int ComElEx3;                     /* compte le nombre d'elements explores dans Algo3 */


MBU static  int * Affecte;

MBU static  int * AdrPere;
              
MBU static  int * PasEncMa;              /* PasEncMa[I]=1 si la machine I n'a pas encore ete exploree */

MBU static  int * PasEncTa;              /* PasEncTa[J]=1 si la tache J n'a pas encore ete exploree  */
  
MBU static  int * MemAdLig;    /* memorise les adresses dans RecLigne des lignes */
                           /* qui ne sont plus obligatoires      */

MBU static  int * TabProvL;

MBU static  int * TabProvC;

MBU static  int * MemAdCol;    /* memorise les adresses dans RecColonne des */
                           /* colonnes qui deviennent obligatoires */

MBU static  int * Arbre;           /* contient la liste des sommmets explores ,cette liste est ordonnee */
...
void OptAffect_HongrAlgo(cIm2D<tINT4> cost,cIm1D<tINT4> res)   /* algorithme "hongrois": */
                                /* "assemble" tous les sous-programmes */
                        

{
  MMVII_INTERNAL_ASSERT_always(cost.DIm().Sz().x()==res.DIm().Sz(),"Hongr : incoherent sizes");
  MMVII_INTERNAL_ASSERT_always(cost.DIm().Sz().y()==res.DIm().Sz(),"Hongr : incoherent sizes");
...
cIm1D<tINT4> OptAffect_HongrAlgo(cIm2D<tINT4> aCost)
{
   cIm1D<tINT4>  aRes(aCost.DIm().Sz().x());
   OptAffect_HongrAlgo(aCost,aRes);
   return aRes;
}
...
void LECTURE(cIm2D<tINT4> cost) 
{
  int I,J;

  MMVII_INTERNAL_ASSERT_always
  (
     cost.DIm().Sz().x() == cost.DIm().Sz().y(),
     "Different size in hongrois"
  );
  Taille = cost.DIm().Sz().x();
  tINT4 ** c = cost.DIm().ExtractRawData2D();
...
{int I,J;
 ...
 int Exact4;
  ...
  MMVII_INTERNAL_ASSERT_always(Exact1==4,"VERIF HONGR" );
...
void LECAFFPRO(cIm1D<tINT4> res)  /* lecture d'une affectation provisoiremnt optimale */


{


   tINT4 * r = res.DIm().ExtractRawData1D();
