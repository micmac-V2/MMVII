...
cOrthoProj::cOrthoProj(const tRotR& aRot, const cPt3dr& aC, const cPt2dr& aPP, tREAL8 aResol, bool profIsZ0)
	: mRL2W(aRot), mC(aC), mPP(aPP), mResol(aResol), mProfIsZ0(profIsZ0)
{
}

cOrthoProj::cOrthoProj(const cOrthoProj& anOP) : cOrthoProj(anOP.mRL2W, anOP.mC, anOP.mPP, anOP.mResol, anOP.mProfIsZ0) {}
...
cCamOrthoC::cCamOrthoC(const std::string& aNameImage, const cOrthoProj& aProj, const cPt2di& aSz)
	: cSensorImage(aNameImage), mProj(aProj),
	  // mDataPixDom  (aSz),
	  mPixelDomain(aSz)
{
}
...
cPt2dr cCamOrthoC::Ground2Image(const cPt3dr& aPGround) const { return Proj(mProj.Value(aPGround)); }
cPt3dr cCamOrthoC::Ground2ImageAndDepth(const cPt3dr& aPGround) const { return mProj.Value(aPGround); }

cPt3dr cCamOrthoC::ImageAndDepth2Ground(const cPt3dr& aPImAndD) const { return mProj.Inverse(aPImAndD); }

tSeg3dr cCamOrthoC::Image2Bundle(const cPt2dr& aPIm) const { return mProj.BundleInverse(aPIm); }
...
cResImagesPPC::cResImagesPPC(const cPt2di& aSz) : mImRadiom(aSz), mImWeight(aSz), mImDepth(aSz) {}
...
cProjPointCloud::cProjPointCloud(cPointCloud& aPC, tREAL8 aWeightInit)
	: mPC(aPC), mNbPtsGlob(aPC.NbPts()), mComputeProfMax(true),
	  // mSurResol  (aSurResol),
	  mAvgD(std::sqrt(1.0 / mPC.CurStdDensity())),
	  // mStepProf  (mAvgD / mSurResol),
	  mSumW(aWeightInit), mSumRad(mNbPtsGlob, 0.0), mImDepth(cPt2di(1, 1)), mDImDepth(nullptr), mImRad(cPt2di(1, 1)),
	  mDImRad(nullptr), mImWeigth(cPt2di(1, 1)), mDImWeigth(nullptr), mImIndex(cPt2di(1, 1)), mDImIndex(nullptr)
...
void cProjPointCloud::SetComputeProfMax(bool aComputeProfMax) { mComputeProfMax = aComputeProfMax; }

void cProjPointCloud::ColorizePC()
{
	StdOut() << ">> ColorizePC called..." << std::endl;
